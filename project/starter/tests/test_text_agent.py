...
"""
# NOTE: when running this test_text_agent test file alone or within the larger suite of test python files in uv run pytest test/ -vv use --junitxml=[report file name] to report text execution results 
...
# creating logger infrastructure
import logging
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("text_agent.log")
    ]
)
...
async def test_moderate_text_has_required_fields():
    ...
    with text_moderation_agent.override(model=TestModel()):
        result = await moderate_text(model, test_text)
        logger.log(level=logging.INFO, msg=f"@test_moderate_text_has_required_fields function result: {result}")
