...
print(f"@image/test_cases.py inserted sys path for evaluators import as: {str(Path(__file__).parent)}")
from evals.image.evaluators import ImageModerationCheck
...
cases: List[Case[List[ImageInput], ImageModerationResult, Any]] = [
    ...
    Case(
        ...
        # Implemented todo : define the evaluators for this case. We need:
        ...
        evaluators=(
            ImageModerationCheck(
                expected_pii=True,  # Implemented todo 
                expected_disturbing=False, # Implemented todo 
                expected_low_quality=True, # Implemented todo 
            ),
            LLMJudge(
                model=judge_model,
                rubric="The rationale should describe specific quality issues (blurry, pixelated, poor exposure, etc.)",
                include_input=True  # Implemented todo : in this case it is probably useful to include the input image for contextue
            ),
        ),
    ),
]
...
async def main():
    ...
    # save to html file 
    with open("eval_image_test_cases_report.html", "w", encoding="utf-8") as f:
