...
print(f"@text/test_cases.py inserted sys path for evaluators import as: {str(Path(__file__).parent)}")
from evals.text.evaluators import TextModerationCheck
...
cases: List[Case[List[TextInput], TextModerationResult, Any]] = [
    # Implemented todo: fill in the missing
    Case(
        ...
        # Implemented todo : fill the parameters for the evaluators for this case. We need:
        ...
        evaluators=(
            ...
            TextModerationCheck(
                expected_pii=False,  # Implemented todo 
                expected_unfriendly=False,  # Implemented todo 
                expected_unprofessional=False,  # TImplemented todo ODO
            ),
            # Use judge model to evaluate if the rationale makes sense
            LLMJudge(
                model=judge_model,  # Implemented todo : add the model to be used as judge (judge_model)
                rubric="The rationale explains why the text is professional and friendly",  # Implemented todo : add a rubric that checks the rationale explains why the text is professional and friendly.
                include_input=True, # Implemented todo : in this case it is probably useful to include the input text for context, so set this to True
            ),
        ),
    ),
    Case(
...
text_dataset = Dataset[List[TextInput], TextModerationResult, Any](
    ...
    # Implemented todo: use the create_repeated_cases function to create the dataset with the test cases defined above
    ...
    cases=create_repeated_cases(
        base_cases=cases,
        num_repeats=int(os.environ.get("EVAL_NUM_REPEATS"))
    ),
    evaluators=[
...
async def main():
    ...
    retry_config = RetryConfig(
        ...
        wait=tenacity.wait_full_jitter(multiplier=0.5, max=15), # creates staggering windows for api retry times instead of uniformly synchronizing all client requests  
    )

    # Run all evaluations

    # Implemented todo: call await text_dataset.evaluate() with the appropriate parameters to enable retries
    ...
    report_string = report.render(include_input=True, include_output=True, include_durations=True)

    # save to html file 
    with open("eval_text_test_cases_report.html", "w", encoding="utf-8") as f:
        f.write("<html><body style='background-color: #ffffff; color: #030000; padding: 20px;'>")
