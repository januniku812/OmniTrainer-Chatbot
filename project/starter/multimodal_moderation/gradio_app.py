...
def check_content_safety(*, text: str | None = None, media: str | None = None) -> Tuple[bool, str, str]:
    ...
    # Implemented todo: use the tracer to create a span named "moderate_text"
    ...
    with tracer.start_as_current_span(
        ...
        ) # seting attribute of generated span object manually via function 
...
class ChatSessionWithTracing:
    ...
    def __init__(self):
        ...
        # Implemented todo: use the tracer to create a span named "conversation" and set an attribute "session.id" with the session_id
    ...
    async def chat_with_gemini(self, message: dict, history: List, past_messages: List) -> Tuple[str, List, str]:
        ...
        # Implemented todo: use the tracer to create a span named "chat_turn"
        ...
        with tracer.start_as_current_span(
            "chat_turn", 
            context=trace.set_span_in_context(self.conversation_span)
        ) as span: # when one chat session in gemini is instantiated it is nested as a chat_turn span within the larger conversation level span that hosts all the chat_turns within itsef
            span.set_attribute(
                SpanAttributes.OPENINFERENCE_SPAN_KIND, "LLM"
            ) # specifies inferece type as llm for phoenix integration 
            ...
            for key, value in message.items():
                ...
                if key == "text" and value:
                    ...
                    # Call moderation backend (FastAPI service) --> satisifes reaction of every turn moderating the 
                    ...
                    if not is_safe:
                        ...
                        # Implemented todo: set an attribute "feedback" in the tracing span with the feedback message
                        # HINT: use span.set_attribute with "feedback" as the key and feedback as the value
                        ...
                        span.set_attribute(
                            "feedback", feedback 
                        ) # we are setting span.set_attribute here with feedback because when content is moderated the loop ends early before sending a message to the LLM customer for a response, hence the return in the next line and set_attribute use here --> otherwise safety_message will be the ai generated feedback itself generated as the turn progresses 
                    ...
                    for file_path in value:
                        ...
                        try:
                            ...
                            if not is_safe:
                                ...
                                span.set_attribute("feedback", feedback) # adding feedback attribute for media flagged spans 
                            ...
                            # Implemented todo: create a BinaryContent object with data=file_bytes and media_type=mime_type
                            # and append it to prompt_parts so it's included in the prompt to the AI
                            
                            file_binary_content = BinaryContent(
                                data=file_bytes,
                                media_type=mime_type 
                            )

                            prompt_parts.append(file_binary_content)
            ...
            # the code will only get here if actual content was provided and content (text/media or both) has passed moderation --> span feedback attribute has not been set at all
            ...
            try:
                with tracer.start_as_current_span("llm_customer"):
                    ...
                    # Implemented todo: use await customer_agent.run to get the result
                    ...
                    logging.log(level=logging.INFO, msg=f"Sending message: {prompt_parts} to LLM customer in @chat_with_gemini")
                    result = await customer_agent.run(
                        prompt_parts,
                        message_history=past_messages, # ensures customer llm responses are contextually consistent with prior responses 
                    )

                span.set_attribute("feedback", (safety_message if safety_message else "Content has passed moderation"))
    ...
    with gr.Blocks(title="ACME Customer Service Training Agent", fill_height=True) as demo:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=3):
                # Implemented todo: fill the missing arguments to gr.ChatInterface
                gr.ChatInterface(
                    ...
                    # Implemented todo: in order to use pydantic AI with Gradio, we need to pass the past_messages_state
...
def main():
    ...
    # ensure that before running gradio_app.main in uv run multimodal-moderation any stale uvicorn fastapi backends at port 8000 are killed so human-ai interactions do not stall after one message --> bash terminal command: fuser -k 8000/tcp 
    # port 7680 - gradio chat interface 
    # port 8000 - fast api backend 
    # port 6006 - phoenix trace UI visibility 
