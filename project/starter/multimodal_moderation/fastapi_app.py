...
app = FastAPI(
    dependencies=[Depends(validate_api_key)] # we are using the fast api service to decouple moderation agent backend and when Swagger UI, the automatic tool for webpage visualization of API, tries to construct frontend, interactive webpage it tries to look for openai.json assuming its at the same root the app is mounted from but in reality it served via a reverse proxy set at a certain endpoint --> therefore the environment variable with the FAST_API_PATH needs to be set to direct the swagger ui openai.json access method to the reverse proxy route 
)
...
# asynchronous moderation agent backend routing to endpoint 
